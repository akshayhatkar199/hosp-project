...
import { unittablesearchByApi } from 'components/Helper/unitmaster';
...
const Unittable = () => {
    ...
    const [UnitExcellsheetdata, setUnitExcellsheetdata] = useState([]);
    ...
    const onSearch = (e) => {
        ...
        getunitbyapi(0, multiplesearchinput);
    };
    ...
    //             setUnitExcellsheetdata(res.data);
    //         },
    //         (err) => {
    //             console.log('err', err);
    //         }
    //     );
    // };
    const getunitbyapi = (offset = 0, multiplesearchinput = '', current = null) => {
        setloading(true);
        ...
        unittablesearchByApi(payloadData).then(
            async (res) => {
                console.log('res=>', res);
                ...
                setUnittableData(res.data.data);
                setloading(false);
            },
            (err) => {
    ...
    const handleTableChange = async (pagination, filters, sorter) => {
        ...
        getunitbyapi(offset, '', pagination.current);
    };
    ...
    return (
        <>
            <Typography variant="h5" className="mx-2">
                <small className="text-black-50">Home</small> {'/'} <b>Unit Details</b>
            </Typography>
            <br></br>
            <MainCard>
                <Grid container spacing={2} columnSpacing={{ sm: 1, md: 3 }}>
                    <Grid item lg={6} xs={6} sm={12} md={6}>
                        <span className="main-title bg-white text-dark ">
                            <b className="brandtaitletext">Unit Details</b>
                        </span>
                    </Grid>
                    ...
                    <Grid item lg={6} xs={12} sm={12} md={6} className="text-start">
                        ...
                        <button className="btn btuniconbord mx-1 m-1 btn-sm" type="button">
                            <CSVLink data={UnitExcellsheetdata} id="excel" className="text-decoration-none text-dark">
                                <FontAwesomeIcon icon={faFileExcel} className="mx-1 " />
                                EXCEL
                            </CSVLink>
                        </button>
                        <button className="btn btuniconbord mx-1 m-1 btn-sm" type="button">
                            {/* <CSVLink data={UnitExcellsheetdata} id="excel" className="text-decoration-none text-white"> */}
                            {/* <FontAwesomeIcon icon={faFileExcel} className="mx-1" /> */}
                            CSV
                            {/* </CSVLink> */}
                        </button>
