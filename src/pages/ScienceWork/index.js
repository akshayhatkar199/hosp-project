...
const ScienceWork = () => {
    ...
    const [ScienceWorkloding, setScienceWorkloding] = useState(false);
    const [scienceWorkData, setScienceWorkData] = useState(null);
    ...
    // console.log('scienceWorkData', scienceWorkData);
        ...
        batmayadetailsGetidByApi(id)
            .then(
                async (res) => {
                    ...
                    setScienceWorkData(response);
                    setCheckbox(response.Checkbox);
                    console.log('response', response);
                    form.resetFields();
                    console.log('res', res);
                    // console.log(' response', response);
                    setScienceWorkloding(false);
                },
                (err) => {
                    console.log('err', err);
                    setScienceWorkloding(false);
    ...
    const onFinishbannerslider = async (values) => {
        ...
        if (typeof id === 'undefined') {
            ...
            var filename = scienceWorkData.imagebatmaya;
            var filename2 = scienceWorkData.image2;
                ...
                >
                    <Typography variant="body2">
                        <Grid container spacing={2}>
                            <Grid item xs={12} lg={4} sm={12} xl={4}>
                                <Form.Item
                                    ...
                                    rules={[
                                        {
                                            required: true,
                                            message: 'Please Enter  ScienceWork Title.'
                                        }
                                    ]}
                                >
                                    <Input name="title1" className="datepicinp textfont" placeholder={'Enter ScienceWork Title'} />
                                </Form.Item>
                                <span className="textlabel">
                                    ScienceWork Title<span className="text-danger">*</span>
                                </span>
                            </Grid>
                            ...
                            <Grid item xs={12} lg={4} sm={12} xl={4}>
                                {id ? (
                                    scienceWorkData && scienceWorkData.imagebatmaya && scienceWorkData.imagebatmaya != '' ? (
                                        <>
                                            <div>
                                                <img
                                                    src={URLidimage + scienceWorkData.imagebatmaya}
                                                    alt=""
