...
import { membertablesearchByApi } from 'components/Helper/membermaster';
...
const Membertable = () => {
    ...
    const columns = [
        {
            ...
            render: (_, record) => (
                <Space size="middle">
                    ...
                    <Link to={`/MemberCreate/${record.id}`}>
    ...
    const petientbyapi = (offset = 0, multiplesearchinput = '', current = null) => {
        ...
        membertablesearchByApi(payloadData).then(
            async (res) => {
                console.log('res=>', res);
                ...
                setMembertableData(res.data.data);
                setloading(false);
            },
            (err) => {
    ...
    return (
        <>
            <Typography variant="h5" className="mx-2">
                <small className="text-black-50">Home</small> {'/'} <b>Member Details</b>
            </Typography>
            <br></br>
            <MainCard>
                <Grid container spacing={2} columnSpacing={{ sm: 1, md: 3 }}>
                    <Grid item lg={6} xs={6} sm={12} md={6}>
                        <span className="main-title bg-white text-dark ">
                            <b className="brandtaitletext">Member Details</b>
                        </span>
                    </Grid>
                    ...
                    <Grid item lg={6} xs={12} sm={12} md={6} className="text-start">
                        ...
                        <button className="btn btuniconbord mx-1 m-1 btn-sm" type="button">
                            <CSVLink data={MemberExcellsheetdata} id="excel" className="text-decoration-none text-dark">
