...
import { employeedetailsGetidByApi, employeedetailsUpdateByApi, employeefromsubmitByApi } from 'components/Helper/employeemaster';
let timeout;
let currentValue;
...
const Employeeform = () => {
    ...
    const [Employeeloding, setEmployeeloding] = useState(false);
    const [employeeetiddata, setemployeiddata] = useState(null);
    ...
    const getemployebyapi = () => {
        setEmployeeloding(true);
        employeedetailsGetidByApi(id)
            .then(
                async (res) => {
                    console.log(' success');
                    let response = res;
                    setemployeiddata(response);
                    console.log('response', response);
                    form.resetFields();
                    console.log('res', res);
                    // console.log(' response', response);
                    setEmployeeloding(false);
                },
                (err) => {
                    console.log('err', err);
                    setEmployeeloding(false);
    ...
    const onFinishemployeiddata = async (values) => {
        console.log('----------', values);
        setloading(true);
        ...
        if (typeof id === 'undefined') {
            ...
            employeefromsubmitByApi(payloadData).then(
                (res) => {
            ...
            setloading(true);
            await employeedetailsUpdateByApi(payloadData)
                .then(
                    async (res) => {
                        console.log('res', res);
                        console.log(' success');
                        swal('employe Update', 'Update success Fully', 'success');
                        navigate('/Employeetable');
                        setloading(false);
                    },
                    (err) => {
                        ...
                        setloading(false);
                        swal('employe', 'Not updated', 'error');
                    }
                )
                .catch();
        }
    ...
    return (
        <>
            ...
            <Card className="driver-create-card ">
                ...
                <Form
                    name="basic"
                    initialValues={employeeetiddata}
                    form={form}
                    onFinish={onFinishemployeiddata}
                    ...
                    <Typography variant="body2">
                        <Grid container spacing={2}>
                            <Grid item xs={12} lg={4} sm={12} xl={4}>
                                ...
                                >
                                    <Input name="employeeName" className="datepicinp textfont" placeholder={'Enter Employee Name'} />
                                </Form.Item>
