...
import './logocreate.css';
// import { LogoCreatedetailsGetidByApi, LogoCreatedetailsUpdateByApi, LogoCreatefromsubmitByApi, uploadImgLogoCreateApi } from 'components/Helper/LogoCreatedetails';
let timeout;
let currentValue;
...
const LogoCreate = () => {
    ...
    const [LogoCreateloding, setLogoCreateloding] = useState(false);
    const [LogoCreateData, setLogoCreateData] = useState(null);
    ...
    // console.log('LogoCreateData', LogoCreateData);
    const URLidimage = 'http://localhost:3000/api/fileuploads/LogoCreate/download/';
    const props = {
        listType: 'picture',
    ...
    useEffect(() => {
        if (typeof id !== 'undefined') {
            getLogoCreatetailbyid();
        } else {
            form.setFieldValue('createdDate', dayjs(new Date()));
            // form.setFieldValue('LogoCreateEndDate', dayjs(new Date()));
        }
    }, [id]);
    const getLogoCreatetailbyid = () => {
        setLogoCreateloding(true);
        LogoCreatedetailsGetidByApi(id)
            .then(
                async (res) => {
                    console.log(' success');
                    let response = res;
                    response.createdDate = dayjs(response.createdDate, dateFormat); // moment(new Date(response.REGI_DATE)).format('YYYY-MM-DD'); // moment();
                    response.LogoCreateEndDate = dayjs(response.LogoCreateEndDate, dateFormat); // moment(new Date(response.REGI_DATE)).format('YYYY-MM-DD'); // moment();
                    setLogoCreateData(response);
                    console.log('response', response);
                    setCheckbox(response.Checkbox);
                    form.resetFields();
                    console.log('res', res);
                    // console.log(' response', response);
                    setLogoCreateloding(false);
                },
                (err) => {
                    console.log('err', err);
                    setLogoCreateloding(false);
    ...
    const onFinishLogoCreateslider = async (values) => {
        console.log('----------', values);
        setloading(true);
        ...
        if (typeof id === 'undefined') {
            ...
            uploadImgLogoCreateApi(formData, 'LogoCreate')
                .then(
                    async (res) => {
                        let filename = '';
                        ...
                        let payloadData = {
                            ...
                            Checkbox: checkboxxx,
                            createdDate: moment(new Date(values.createdDate)).format('DD-MM-YYYY')
                        };
                        console.log('payloaddata', payloadData);
                        LogoCreatefromsubmitByApi(payloadData).then(
                            (res) => {
                                console.log('resLogoCreate', res);
            ...
            var filename = LogoCreateData.imageupload;
            // setBnnerPhoto(filename);
            const formData = new FormData();
            if (values.image && values.image.file) {
                formData.append('files', values.image.file);
                console.log('plase apload imag');
                await uploadImgLogoCreateApi(formData, 'LogoCreate').then(async (res) => {
                    if (res.result && res.result.files && res.result.files.files.length > 0) {
                        filename = res.result.files.files[0].name;
                    }
                });
            ...
            setloading(true);
            await LogoCreatedetailsUpdateByApi(payloadData)
                .then(
                    async (res) => {
                        console.log('res', res);
                        console.log(' success');
    ...
    const onFinishFailedLogoCreateslider = (value) => {
    ...
    return (
        <>
            ...
            <Card className="driver-create-card ">
                ...
                <Form
                    name="basic"
                    initialValues={LogoCreateData}
                    // layout="vertical"
                    // labelCol={{ span: 22 }}
                    // wrapperCol={{ span: 22 }}
                    form={form}
                    onFinish={onFinishLogoCreateslider}
                    onFinishFailed={onFinishFailedLogoCreateslider}
                    autoComplete="off"
                >
                    <Typography variant="body2">
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12} lg={4} sm={12} xl={4}>
                                {id ? (
                                    LogoCreateData && LogoCreateData.imageupload && LogoCreateData.imageupload != '' ? (
                                        <>
                                            <div>
                                                <img
                                                    src={URLidimage + LogoCreateData.imageupload}
                                                    alt=""
