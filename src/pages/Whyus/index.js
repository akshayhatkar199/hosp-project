...
import './whyus.css';
// import { WhyusdetailsGetidByApi, WhyusdetailsUpdateByApi, WhyusfromsubmitByApi, uploadImgWhyusApi } from 'components/Helper/Whyusdetails';
let timeout;
let currentValue;
...
const Whyus = () => {
    ...
    const [whyusloding, setwhyusloding] = useState(false);
    ...
    const URLidimage = 'http://localhost:3000/api/fileuploads/Whyus/download/';
    const props = {
        listType: 'picture',
    ...
    useEffect(() => {
        if (typeof id !== 'undefined') {
            ...
            // form.setFieldValue('WhyusEndDate', dayjs(new Date()));
        }
    }, [id]);
    const getWhyustailbyid = () => {
        setwhyusloding(true);
        WhyusdetailsGetidByApi(id)
            .then(
                async (res) => {
                    console.log(' success');
                    let response = res;
                    response.createdDate = dayjs(response.createdDate, dateFormat); // moment(new Date(response.REGI_DATE)).format('YYYY-MM-DD'); // moment();
                    response.WhyusEndDate = dayjs(response.WhyusEndDate, dateFormat); // moment(new Date(response.REGI_DATE)).format('YYYY-MM-DD'); // moment();
                    setwhyusData(response);
                    console.log('response', response);
                    setCheckbox(response.Checkbox);
                    form.resetFields();
                    console.log('res', res);
                    // console.log(' response', response);
                    setwhyusloding(false);
                },
                (err) => {
                    console.log('err', err);
                    setwhyusloding(false);
    ...
    const onFinishWhyusslider = async (values) => {
        console.log('----------', values);
        setloading(true);
        ...
        if (typeof id === 'undefined') {
            ...
            uploadImgWhyusApi(formData, 'Whyus')
                .then(
                    async (res) => {
                        let filename = '';
                        ...
                        WhyusfromsubmitByApi(payloadData).then(
                            (res) => {
                                console.log('resWhyus', res);
            ...
            var filename = whyusData.imageupload;
            // setBnnerPhoto(filename);
            const formData = new FormData();
            if (values.image && values.image.file) {
                formData.append('files', values.image.file);
                console.log('plase apload imag');
                await uploadImgWhyusApi(formData, 'Whyus').then(async (res) => {
                    if (res.result && res.result.files && res.result.files.files.length > 0) {
                        filename = res.result.files.files[0].name;
                    }
                });
            ...
            setloading(true);
            await WhyusdetailsUpdateByApi(payloadData)
                .then(
                    async (res) => {
                        console.log('res', res);
                        console.log(' success');
    ...
    const onFinishFailedWhyusslider = (value) => {
    ...
    return (
        <>
            ...
            <Card className="driver-create-card ">
                ...
                <Form
                    name="basic"
                    initialValues={whyusData}
                    // layout="vertical"
                    // labelCol={{ span: 22 }}
                    // wrapperCol={{ span: 22 }}
                    form={form}
                    onFinish={onFinishWhyusslider}
                    onFinishFailed={onFinishFailedWhyusslider}
                    autoComplete="off"
                >
                    <Typography variant="body2">
                        <Grid container spacing={2}>
                            <Grid item xs={12} lg={4} sm={12} xl={4}>
                                <Form.Item
                                    ...
                                    rules={[
                                        {
                                            required: true,
                                            message: 'Please Whyus Lable.'
                                        }
                                    ]}
                                >
                                    <TextArea row={2} name="title1" className="datepicinp textfont" placeholder={'Enter Whyus Title'} />
                                </Form.Item>
                                <span className="textlabelbatmaya">
                                    Whyus Title<span className="text-danger">*</span>
                                </span>
                            </Grid>
                            ...
                            <Grid item xs={12} lg={4} sm={12} xl={4}>
                                {id ? (
                                    whyusData && whyusData.imageupload && whyusData.imageupload != '' ? (
                                        <>
                                            <div>
                                                <img
                                                    src={URLidimage + whyusData.imageupload}
                                                    alt=""
