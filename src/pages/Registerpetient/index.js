...
const Registerpetient = () => {
    const conponentPDF = useRef();
    let { offset } = useParams();
    const [RegisterpetientData, setRegisterpetientData] = useState([]);
    ...
    const petientbyapi = (offset = 0, current = null) => {
        setloading(true);
        let payloadData = {
            offset: offset,
            limit: 10,
        ...
        membertableshowsearchByApi(payloadData).then(
            async (res) => {
                // console.log('res=>', res);
                // console.log('current----------', current);
                setRegisterpetientData(res.data.data);
                setloading(false);
            },
            (err) => {
                console.log(err);
    ...
    return (
        <>
            <Typography variant="h5" className="mx-2">
                <small className="text-black-50">Home</small> {'/'} <b>Pationt Details Lists Print</b>
            </Typography>
            <br></br>
            <MainCard>
...
export default Registerpetient;
