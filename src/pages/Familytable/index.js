...
import { DetailListByApi, detailsExcellsheetByApi } from '../../components/Helper/details';
import { familyrelationtablesearchByApi } from 'components/Helper/familyrelationmaster';
...
const Familytable = () => {
    ...
    const [FamilyExcellsheetdata, setFamilyExcellsheetdata] = useState([]);
    ...
    const columns = [
        {
            ...
            render: (_, record) => (
                <Space size="middle">
                    ...
                    <Link to={`/FamilyCreate/${record.id}`}>
    ...
    const onSearch = (e) => {
        ...
        familybyapi(0, multiplesearchinput);
    };
    ...
    //             setFamilyExcellsheetdata(res.data);
    //         },
    //         (err) => {
    //             console.log('err', err);
    //         }
    //     );
    // };
    const familybyapi = (offset = 0, multiplesearchinput = '', current = null) => {
        setloading(true);
        ...
        familyrelationtablesearchByApi(payloadData).then(
            async (res) => {
                console.log('res=>', res);
                ...
                setFamilytableData(res.data.data);
                setloading(false);
            },
            (err) => {
    ...
    const handleTableChange = async (pagination, filters, sorter) => {
        ...
        familybyapi(offset, '', pagination.current);
    };
    ...
    return (
        <>
            <Typography variant="h5" className="mx-2">
                <small className="text-black-50">Home</small> {'/'} <b>Family Details</b>
            </Typography>
            <br></br>
            <MainCard>
                <Grid container spacing={2} columnSpacing={{ sm: 1, md: 3 }}>
                    <Grid item lg={6} xs={6} sm={12} md={6}>
                        <span className="main-title bg-white text-dark ">
                            <b className="brandtaitletext">Family Details</b>
                        </span>
                    </Grid>
                    <Grid item lg={6} xs={12} sm={12} md={6} className="text-end">
                        <Link to="/FamilyCreate">
                    ...
                    <Grid item lg={6} xs={12} sm={12} md={6} className="text-start">
                        ...
                        <button className="btn btuniconbord mx-1 m-1 btn-sm" type="button">
                            <CSVLink data={FamilyExcellsheetdata} id="excel" className="text-decoration-none text-dark">
                                <FontAwesomeIcon icon={faFileExcel} className="mx-1 " />
                                EXCEL
                            </CSVLink>
                        </button>
                        <button className="btn btuniconbord mx-1 m-1 btn-sm" type="button">
                            {/* <CSVLink data={FamilyExcellsheetdata} id="excel" className="text-decoration-none text-white"> */}
                            {/* <FontAwesomeIcon icon={faFileExcel} className="mx-1" /> */}
                            CSV
                            {/* </CSVLink> */}
                        </button>
